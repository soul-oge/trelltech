...
import { useArchiveList, useCreateList, useLists, useUpdateListName } from '@app/features/lists/hooks';
import { useState } from 'react';
import { ScrollView, Text, TextInput, TouchableOpacity, View } from 'react-native';

export default function SandboxBoardScreen() {
  const [boardId, setBoardId] = useState('demo-board'); // <- seed du mock
  ...
  return (
    <View style={{ flex:1, backgroundColor:'#F3F4F6', padding:12 }}>
      ...
      <View style={{ flexDirection:'row', gap:8, marginBottom:10 }}>
        ...
        />
        <TouchableOpacity onPress={() => setBoardId(boardId.trim())}>
          <Text style={{ color:'#2563EB', padding:10 }}>Charger</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={{ flexDirection:'row', gap:8, marginBottom:10 }}>
        <TextInput
          ...
          style={{ flex:1, backgroundColor:'white', borderRadius:10, paddingHorizontal:10, borderWidth:1, borderColor:'#E5E7EB' }}
        />
        <TouchableOpacity onPress={() => { if (newList.trim()) { createList.mutate(newList.trim()); setNewList(''); }}}>
